...
class SendScheduleAssignmentUpdatedNotification implements ShouldQueue
{
    ...
    public function handle(ScheduleAssignmentUpdated $event): void
    {
        ...
        $payload = [
            ...
            'start_time' => $assignment->start_time instanceof \Carbon\CarbonInterface ? $assignment->start_time->format('H:i') : 'N/A',
            'end_time' => $assignment->end_time instanceof \Carbon\CarbonInterface ? $assignment->end_time->format('H:i') : 'N/A',
            'lunch' => $assignment->lunch_start_time instanceof \Carbon\CarbonInterface ? $assignment->lunch_start_time->format('H:i') . ' - ' . $assignment->lunch_end_time->format('H:i') : 'No programado',
            'break' => $assignment->break_start_time instanceof \Carbon\CarbonInterface ? $assignment->break_start_time->format('H:i') . ' - ' . $assignment->break_end_time->format('H:i') : 'No programado',
