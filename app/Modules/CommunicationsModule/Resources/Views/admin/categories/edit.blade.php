...
@section('title', 'Editar Categoría - Comunicaciones')
...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center mb-8">
            ...
            <h1 class="text-3xl font-bold text-slate-900">Editar Categoría</h1>
        </div>

        <div class="bg-white shadow-md rounded-md p-4">
            <form method="POST" action="{{ route('communications.admin.categories.update', $category) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description"
                              name="description"
                              rows="3"
                              class="w-full px-3 py-2 border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-950 @error('description') border-red-500 @enderror">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-8">
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm text-slate-700">Categoría activa</span>
                    </label>
                </div>
