<div class="fixed top-4 right-4 z-[100] flex flex-col gap-4 w-full max-w-sm pointer-events-none" x-data="{
        remove(id) {
            $wire.removeToast(id);
        }
    ...
    @foreach ($toasts as $toast)
        <div wire:key="{{ $toast['id'] }}" x-data="{ show: true }" x-show="show"
            ...
            class="pointer-events-auto group relative flex items-start gap-3 overflow-hidden rounded-md border border-slate-200 bg-white dark:bg-slate-900 p-4 shadow-md dark:border-slate-700">
            ...
            <button type="button" @click="show = false; setTimeout(() => remove('{{ $toast['id'] }}'), 300)"
                class="ml-auto shrink-0 text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300">
