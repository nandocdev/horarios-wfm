<div class=" mx-auto space-y-8">
            ...
            <div class="flex items-center gap-2 mb-2">
                ...
                @php
                    $statusColor = match ($ticket->status) {
                        ...
                        'on_hold' => 'slate', 'resolved' => 'green', 'closed' => 'zinc', default => 'zinc'
                    };
                @endphp
                ...
                @endif
                <span class="text-xs text-slate-500 ml-2">#{{ str_pad($ticket->id, 5, '0', STR_PAD_LEFT) }}</span>
            </div>
            ...
            @if(!$comment->is_internal || ($comment->is_internal && $isSupport))
                <div class="flex items-start gap-4 {{ $isAuthorMe ? 'flex-row-reverse' : '' }}">
                    <flux:avatar :name="$comment->author->first_name . ' ' . $comment->author->last_name" size="sm"
                        class="{{ $comment->is_internal ? 'ring-2 ring-amber-500' : '' }}" />
                    ...
                    <div class="flex flex-col {{ $isAuthorMe ? 'items-end' : 'items-start' }} max-w-[80%]">
                        ...
                        @php
                            $bubbleClasses = match (true) {
                                ...
                                default => 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-tl-none'
                            };
                        @endphp
    ...
    @if(!in_array($ticket->status, ['resolved', 'closed']))
        <flux:card class="mt-6 border-slate-200 dark:border-slate-700">
    ...
    @else
        <flux:card class="mt-6 bg-slate-50 dark:bg-slate-900/30 text-center border-dashed">
            <flux:icon icon="lock-closed" class="mx-auto size-8 text-slate-400 mb-2" />
