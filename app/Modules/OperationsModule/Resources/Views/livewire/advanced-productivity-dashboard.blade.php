<div class="p-6 space-y-8 bg-slate-50 min-h-screen">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <flux:card>
            ...
            <p class="text-4xl font-black text-slate-800 mt-2">{{ number_format($summary['total_work_units'], 0) }}</p>
        ...
        <flux:card>
            ...
            <p class="text-4xl font-black text-slate-800 mt-2">{{ number_format($summary['total_capacity_calls'], 0) }}</p>
                    ...
                    @foreach($topPerformers as $m)
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <p class="text-xs font-bold text-slate-700 truncate">{{ $m->employee->full_name }}</p>
                                    <p class="text-[10px] text-slate-400">{{ $m->employee->team?->name }}</p>
                                </div>
                            </div>
                            <flux:badge color="green" inset="top bottom">{{ number_format($m->pwi_pct, 1) }}%</flux:badge>
                        </div>
                    @endforeach
                </div>
            </flux:card>
                ...
                <div class="space-y-3">
                    @foreach($underPerformers as $m)
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                            ...
                            <flux:badge color="red" inset="top bottom">{{ number_format($m->efficiency_pct, 1) }}% Eff.</flux:badge>
                        </div>
                    @endforeach
                </div>
            </flux:card>
                    ...
                    <table class="w-full text-left">
                        <thead class="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <tr>
                                ...
                                <th class="p-4 text-center">Real</th>
                                <th class="p-4 text-center">PWI</th>
                                <th class="p-4 text-center">WU</th>
                                <th class="p-4 text-center">AHTw</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="p-4">
                                        <div class="flex items-center gap-3">
                                            <flux:avatar src="{{ $m->employee->avatar_url }}" size="xs" />
                                            <div>
                                                <p class="font-bold text-slate-700">{{ $m->employee->full_name }}</p>
                                    ...
                                    <td class="p-4 text-center">
                                        <div class="flex flex-col items-center">
                                            <span class="font-bold text-slate-600">{{ number_format($m->capacity_calls, 1) }}</span>
                                            <span class="text-[9px] text-slate-400 uppercase">Esperadas</span>
                                        </div>
                                    </td>
                                    <td class="p-4 text-center">
                                        <div class="flex flex-col items-center">
                                            <span class="font-black text-slate-800">{{ $m->calls_total }}</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="space-y-4">
                <div class="p-4 bg-indigo-50 rounded-xl border border-indigo-100 shadow-sm">
                    ...
                    <p class="text-[11px] text-indigo-700 leading-relaxed italic mb-2">Availability % × Efficiency %</p>
