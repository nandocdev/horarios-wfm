    ...
    <div class="flex-none bg-slate-50 p-4 rounded-md border border-slate-100">
        <div class="flex justify-between items-center mb-4">
            ...
            <div class="flex flex-wrap gap-x-4 gap-y-2 text-[10px] font-medium text-slate-400">
                <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-sm bg-blue-600"></span> Talking</div>
                <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-sm bg-green-600"></span> Ready</div>
                <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-sm bg-red-600"></span> Not Ready</div>
                <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-sm bg-slate-600"></span> Reserved</div>
                <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-sm bg-amber-500"></span> Work / Break</div>
                <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-sm bg-slate-500"></span> Logged-in</div>
                <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-sm bg-slate-950"></span> Logout</div>
            </div>
        </div>

        <div class="relative h-4 bg-slate-200 rounded-md overflow-hidden shadow-inner">
                ...
                @forelse($timeline as $item)
                    ...
                    @if($lastHour !== $currentHour)
                        <div class="relative z-10 flex items-center gap-4 pt-4 pb-2">
                            <div class="w-6 h-6 rounded-md bg-white border-2 border-slate-200 flex items-center justify-center">
                                <span class="text-[9px] font-bold text-slate-400">{{ explode(':', $currentHour)[0] }}</span>
                            </div>
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">{{ $currentHour }}</span>
                        </div>
                    ...
                    <div class="relative z-10 flex items-start gap-4 group">
                        <div class="mt-1.5 w-6 flex justify-center">
                            <div class="w-2 h-2 rounded-sm {{ $item->isCurrent ? 'bg-blue-600 animate-pulse' : ($item->isReal ? 'bg-slate-300' : 'border border-slate-300 bg-white') }}"></div>
                        </div>

                        <div class="flex-1 bg-white p-4 rounded-md border border-slate-100 shadow-sm group-hover:border-slate-200 transition-opacity">
                            <div class="flex items-center justify-between mb-1">
                                <div class="flex items-center gap-2">
                                    <flux:badge :color="$item->color" variant="subtle" size="sm" class="rounded-md font-bold uppercase tracking-wide">
                                        {{ $item->label }}
                                    </flux:badge>
                                    
                                    @if($item->isReal)
                                        <flux:badge size="xs" color="slate" variant="subtle" class="rounded-md">Real</flux:badge>
                                    @endif
                                    
                                    @if($item->isCurrent)
                                        <span class="flex h-2 w-2 rounded-sm bg-blue-600 animate-pulse" title="Actual"></span>
                                    @endif
                                </div>
                                <span class="text-[10px] font-mono font-semibold text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded">
                                    {{ $item->displayTime }}
                                </span>
                            </div>
