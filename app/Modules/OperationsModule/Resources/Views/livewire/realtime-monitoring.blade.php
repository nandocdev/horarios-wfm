<div wire:poll.10s class="space-y-8">
    ...
    <div
        ...
        <div class="space-y-1">
            <div class="flex items-center gap-3">
                ...
                <flux:heading size="xl" level="1" class="font-bold tracking-tight">Monitoreo en Tiempo Real
                </flux:heading>
                {{-- Reloj del Servidor --}}
                <div x-data="{ 
            ...
            }" class="flex items-center gap-4 pl-6 border-l border-slate-200 dark:border-slate-800">
                    <div class="flex flex-col items-end">
                        <div class="flex items-center gap-2 text-slate-900 dark:text-white">
                            <span class="text-3xl font-bold font-mono tracking-tighter tabular-nums"
                                x-text="serverTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })"></span>
                        </div>
                        <flux:text size="xs"
                            class="uppercase font-semibold text-slate-400 tracking-widest whitespace-nowrap leading-none">
                            Hora del Servidor</flux:text>
                    </div>
                    <div class="p-2.5 bg-slate-100 dark:bg-slate-800 rounded-md">
                        <flux:icon name="clock" class="size-6 text-slate-500" variant="micro" />
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 pl-11">
                @if($stats['worker_active'])
                    ...
                    <flux:text size="xs" class="text-slate-500 dark:text-zinc-400 font-medium">Sincronización Activa • <span
                            class="text-slate-800 dark:text-slate-200">{{ $stats['worker_last_update'] }}</span></flux:text>
                @else
                    <div class="relative flex h-2 w-2">
        ...
        <div class="flex flex-col xl:flex-row items-center gap-4">
            <div class="flex flex-wrap items-center gap-2">
                {{-- Stat: Total --}}
                <div
                    class="flex flex-col items-center px-4 py-2 bg-slate-50 border border-slate-200 dark:border-zinc-800 min-w-[80px]">
                    <flux:text size="xs" class="uppercase font-semibold text-slate-400 tracking-widest">Total</flux:text>
                    <flux:text size="lg" class="font-black text-slate-900 dark:text-zinc-100 leading-none mt-1">
                        {{ $stats['total'] }}
                    </flux:text>
                </div>
                ...
                {{-- Stat: Listos --}}
                <div
                    class="flex flex-col items-center px-4 py-2 bg-green-50 border border-green-200 rounded-md min-w-[80px]">
                    <flux:text size="xs"
                        class="uppercase font-bold text-green-600 dark:text-green-400 tracking-widest">Listos
                    </flux:text>
                    <flux:text size="lg" class="font-bold text-green-600 dark:text-green-500 leading-none mt-1">
                        {{ $stats['ready'] }}
                    </flux:text>
                </div>
                ...
                {{-- Stat: En Llamada --}}
                <div
                    class="flex flex-col items-center px-4 py-2 bg-blue-50 border border-blue-200 rounded-md min-w-[80px]">
                    <flux:text size="xs" class="uppercase font-bold text-blue-600 dark:text-blue-400 tracking-widest">
                        Hablando</flux:text>
                    <flux:text size="lg" class="font-bold text-blue-600 dark:text-blue-500 leading-none mt-1">
                        {{ $stats['talking'] }}
                    </flux:text>
                </div>
                ...
                {{-- Stat: No Listos --}}
                <div
                    class="flex flex-col items-center px-4 py-2 bg-amber-50 border border-amber-200 rounded-md min-w-[80px]">
                    <flux:text size="xs" class="uppercase font-bold text-amber-600 dark:text-amber-400 tracking-widest">
                        Aux</flux:text>
                    <flux:text size="lg" class="font-bold text-amber-500 dark:text-amber-500 leading-none mt-1">
                        {{ $stats['not_ready'] }}
                    </flux:text>
                </div>
                ...
                {{-- Stat: Ausentes --}}
                <div
                    class="flex flex-col items-center px-4 py-2 bg-red-50 border border-red-200 rounded-md min-w-[80px]">
                    <flux:text size="xs" class="uppercase font-bold text-red-600 dark:text-red-400 tracking-widest">
                        Ausentes</flux:text>
                    <flux:text size="lg" class="font-bold text-red-600 dark:text-red-500 leading-none mt-1">
                        {{ $stats['absent_count'] }}
                    </flux:text>
                </div>
                ...
                {{-- Stat: Desconectados --}}
                <div
                    class="flex flex-col items-center px-4 py-2 bg-slate-50 border border-slate-200 rounded-md min-w-[80px]">
                    <flux:text size="xs"
                        class="uppercase font-bold text-slate-600 dark:text-slate-400 tracking-widest">Desconec
                    </flux:text>
                    <flux:text size="lg" class="font-bold text-slate-600 dark:text-slate-300 leading-none mt-1">
                ...
                {{-- Stat: Adherencia --}}
                <div
                    class="flex flex-col items-center px-4 py-2 {{ $stats['adherence_percent'] < 85 ? 'bg-red-50 dark:bg-red-900/10 border-red-100 dark:border-red-800/20' : 'bg-blue-50 border border-blue-200' }} rounded-md border min-w-[100px]">
                    <flux:text size="xs"
                    ...
                    <flux:text size="lg"
                        class="font-black {{ $stats['adherence_percent'] < 85 ? 'text-red-700 dark:text-red-500' : 'text-blue-700 dark:text-blue-500' }} leading-none mt-1">
    ...
    <flux:card>
        <div class="space-y-4">
            ...
            <flux:table>
                <flux:table.columns>
                    <flux:table.column class="sticky top-0 z-10" sortable :sorted="$sortField === 'agent'" :direction="$sortDirection"
                        wire:click="sortBy('agent')">Agente</flux:table.column>

                    <flux:table.column>Estado Programado</flux:table.column>
                    <flux:table.column class="sticky top-0 z-10" sortable :sorted="$sortField === 'state'" :direction="$sortDirection"
                        wire:click="sortBy('state')">Estado Real</flux:table.column>
                    <flux:table.column>Alertas Operativas</flux:table.column>
                    <flux:table.column class="sticky top-0 z-10" sortable :sorted="$sortField === 'duration'" :direction="$sortDirection"
                        wire:click="sortBy('duration')" align="end">Duración</flux:table.column>
                    <flux:table.column class="sticky top-0 z-10" align="end">Acciones</flux:table.column>
                </flux:table.columns>
                ...
                <flux:table.rows>
                    @forelse($agents as $agent)
                        ...
                        @endphp
                        <flux:table.row class="hover:bg-slate-50 transition-colors duration-150" :key="$agent->id"
                            ...
                            <flux:table.cell class="py-2">
                                <div class="flex items-center gap-3">
                                    <div class="relative">
                                        ...
                                        <div class="absolute -bottom-1 -right-1">
                                            @if($agent->is_adherent)
                                                <div
                                                    class="flex items-center justify-center w-4 h-4 rounded-full bg-white dark:bg-slate-900 shadow-sm">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="font-bold text-sm text-slate-900 dark:text-white leading-tight">
                                            {{ $agent->first_name }} {{ $agent->last_name }}
                                        </span>
                                        <span
                                            class="text-[10px] font-medium text-slate-500 uppercase tracking-tighter">{{ $agent->position_name ?? 'Sin cargo' }}
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        @if(isset($agent->expected_state['start_time']) && isset($agent->expected_state['end_time']))
                                            <div class="ml-4 leading-none">
                                                <flux:text size="xs" class="text-zinc-400 dark:text-slate-500 font-mono">
                            ...
                            <flux:table.cell class="py-2">
                                <div class="flex flex-col gap-1">
                                    <flux:badge :color="$color" size="sm" class="font-semibold w-fit rounded-md">
                                        {{ $displayName }}
                                    </flux:badge>
                                    ...
                                    @elseif($agent->reason_code && !in_array($agent->current_state, ['READY', 'TALKING']))
                                        <span class="text-[10px] text-slate-500 italic truncate max-w-[120px]">
                                            {{ $agent->reason_code }}
                                        </span>
                                    @endif
                                </div>
                            ...
                            <flux:table.cell align="end">
                                <div class="flex flex-col items-end">
                                    <span
                                        class="font-mono font-semibold text-sm {{ count($agent->alerts) > 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-900 dark:text-white' }} tabular-nums">
                                        {{ $durationFormatted }}
                                    </span>
                                </div>
                            </flux:table.cell class="py-2">
                        ...
                        <flux:table.row>
                            <flux:table.cell colspan="5" class="text-center py-20">
                                <div class="flex flex-col items-center">
                                    <div class="p-4 bg-zinc-50 dark:bg-slate-900 rounded-full mb-4">
                                        <flux:icon icon="users" class="size-10 text-slate-300 dark:text-slate-700" />
                                    </div>
    ...
    <flux:modal name="agent-details-modal" class="md:w-[600px] space-y-4">
        @if($selectedAgent)
            ...
            <div class="grid grid-cols-2 gap-4">
                <flux:card class="p-3 bg-zinc-50 dark:bg-slate-900/50">
                    <flux:text size="xs" class="uppercase font-bold text-slate-500">Estado Actual</flux:text>
                    <div class="flex items-center gap-2 mt-1">
                        <flux:badge size="sm" class="rounded-md">{{ $selectedAgent->current_state }}</flux:badge>
                        ...
                        @if($selectedAgent->current_queue && $selectedAgent->current_state === 'TALKING')
                            <flux:badge size="sm" class="rounded-md" color="blue" icon="phone">{{ $selectedAgent->current_queue }}</flux:badge>
                        @endif
                    </div>
                </flux:card>
                <flux:card class="p-3 bg-zinc-50 dark:bg-slate-900/50">
                    <flux:text size="xs" class="uppercase font-bold text-slate-500">Último Cambio</flux:text>
            ...
            <flux:card class="p-4 border-l-4" style="border-left-color: {{ $expected['color'] ?? '#6b7280' }}">
                <div class="flex justify-between items-start">
                    <div>
                        <flux:text size="xs" class="uppercase font-bold text-slate-500">Expectativa de Turno</flux:text>
                        <flux:heading size="sm" class="mt-1">{{ $expected['label'] }}</flux:heading>
                    </div>
                    @if(isset($expected['start_time']) && isset($expected['end_time']))
                        <div class="text-right">
                            <flux:text size="xs" class="uppercase font-bold text-slate-500">Horario</flux:text>
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                    @forelse($agentEvents as $event)
                        <div
                            class="flex items-center justify-between p-2 rounded-md border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-slate-900">
                            <div class="flex items-center gap-3">
                                <div class="w-2 h-2 rounded-sm bg-zinc-300 dark:bg-zinc-600"></div>
                                <div class="flex flex-col">
                                    <flux:text size="sm" font="bold">{{ $event->agent_state }}</flux:text>
                                    <flux:text size="xs" class="text-slate-500">{{ $event->reason_code ?? 'Sin motivo' }}
                                    </flux:text>
                                </div>
                            </div>
                            <div class="flex flex-col items-end">
                    ...
                    @empty
                        <flux:text size="sm" class="text-center py-4 text-slate-500">No hay eventos registrados para hoy.
                        </flux:text>
                    @endforelse
                </div>
            </div>
