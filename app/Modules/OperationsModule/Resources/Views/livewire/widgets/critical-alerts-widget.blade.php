<flux:card>
    ...
    <div class="space-y-4">
        ...
        @if($pendingApprovals > 0)
            <div class="p-4 bg-blue-50 dark:bg-blue-950/20 border-l-4 border-blue-600 flex items-start gap-3">
                <flux:icon name="clock" class="text-blue-500 mt-0.5" />
                <div>
                    <flux:text size="sm" class="font-semibold text-slate-900 dark:text-blue-200">
        ...
        @foreach($scheduleConflicts as $conflict)
            <div class="p-4 bg-amber-50 dark:bg-amber-950/20 border-l-4 border-amber-500 flex items-start gap-3">
                <flux:icon name="shield-exclamation" variant="solid" class="text-amber-500 mt-0.5 w-5 h-5" />
                <div>
                    <flux:text size="sm" class="font-semibold text-slate-900 dark:text-amber-200">
        ...
        @foreach($criticalQueues as $queue)
            <div class="p-4 bg-red-50 dark:bg-red-950/20 border-l-4 border-red-600 flex items-start gap-3">
                <flux:icon name="exclamation-triangle" class="text-red-500 mt-0.5" />
                <div>
                    <flux:text size="sm" class="font-semibold text-slate-900 dark:text-red-200">
        ...
        @if($pendingApprovals === 0 && empty($scheduleConflicts) && empty($criticalQueues))
             <div class="flex flex-col items-center justify-center py-8 text-center">
                <flux:icon name="check-circle" class="w-10 h-10 text-green-600 mb-2" />
                <flux:text size="sm" class="text-slate-500">No hay alertas críticas en este momento.</flux:text>
            </div>
        @endif
    </div>
</flux:card>
