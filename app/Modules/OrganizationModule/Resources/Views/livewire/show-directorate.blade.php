        ...
        <div class="p-4 border-b border-slate-200">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <h1 class="text-3xl font-bold text-slate-900">{{ $directorate->name }}</h1>
                </div>
                ...
                <div>
                    ...
                    <dl class="space-y-3">
                        <div>
                            ...
                            <dd class="text-sm text-slate-900">{{ $directorate->name }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-slate-500">Descripción</dt>
                            <dd class="text-sm text-slate-900">{{ $directorate->description ?: 'Sin descripción' }}</dd>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <dd class="text-sm text-slate-900">{{ $directorate->created_at->format('d/m/Y H:i') }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-slate-500">Última actualización</dt>
                            <dd class="text-sm text-slate-900">{{ $directorate->updated_at->format('d/m/Y H:i') }}</dd>
                        </div>
                    </dl>
                </div>

                <div>
                    <h3 class="text-xl font-semibold text-slate-900 mb-4">Departamentos
                            ...
                            @foreach($directorate->departments as $department)
                                <div class="flex items-center justify-between p-4 bg-slate-50 rounded-md">
                                    <div>
                                        ...
                                        <div class="text-sm text-slate-500">{{ $department->description ?: 'Sin descripción' }}
                                        </div>
                                    </div>
                    ...
                    @else
                        <p class="text-sm text-slate-500">No hay departamentos asociados.</p>
                    @endif
                </div>
            </div>
        </div>
