            ...
            <div class="space-y-4">
                ...
                @if(!empty($importedData))
                    <div class="mt-6 border-t pt-6 border-slate-200">
                        <flux:heading size="md" class="mb-2">{{ __('Días a aplicar (Periodo de implementación)') }}
                        </flux:heading>
                            ...
                            <flux:table class="m-4">
                                ...
                                <flux:table.rows>
                                    @foreach($importedData as $index => $row)
                                        <flux:table.row :key="$row['id']" class="hover:bg-slate-50"
                                            :class="!$row['user_exists'] ? 'bg-red-50/50' : ''">
                                            <flux:table.cell class="py-1">
                                                <div class="flex flex-col gap-1">
                                                    <flux:text class="w-28" size="sm">{{ strtolower($row['usuario']) }}
                                                    </flux:text>
                                                    @if(!$row['user_exists'])
                                            ...
                                            <flux:table.cell class="py-1">
                                                <flux:select wire:model="importedData.{{ $index }}.entrada" size="sm"
                                                    class="w-44">
                                            ...
                                            <flux:table.cell class="py-1">
                                                <flux:input type="time" wire:model="importedData.{{ $index }}.salida"
                                                    size="sm" />
                                            </flux:table.cell>
                                            <flux:table.cell class="py-1">
                                            ...
                                            <flux:table.cell align="end" class="py-1">
                                                <flux:button wire:click="removeImportedRow({{ $index }})" variant="danger"
                                                    size="sm" icon="trash" />
                                            </flux:table.cell>
                                        </flux:table.row>
                                    @endforeach
                ...
                <div class="flex justify-end gap-3 mt-6 border-t pt-4 border-slate-200">
                    <flux:button variant="ghost" href="{{ route('schedules.planning.teams', ['week' => $week->id]) }}"
                        wire:navigate>
                        {{ __('Cancelar') }}
                    </flux:button>
                    @if(!empty($importedData))
