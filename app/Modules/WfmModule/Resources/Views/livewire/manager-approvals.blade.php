        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b bg-slate-50 dark:bg-slate-900/50 sticky top-0 z-10">
                    <th class="py-2 px-4 text-xs font-bold uppercase tracking-wider text-slate-500">Empleado</th>
                    <th class="py-2 px-4 text-xs font-bold uppercase tracking-wider text-slate-500">Tipo / Fecha</th>
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-opacity">
                        <td class="py-2 px-4 text-sm">
                            <div class="flex flex-col">
                                ...
                                <span class="text-xs text-slate-500">{{ $request->employee->position->name ?? 'N/A' }}</span>
                            </div>
                        </td>
                        <td class="py-2 px-4 text-sm">
                            <div class="flex flex-col">
                                <span class="font-medium text-primary-600">{{ ucfirst($request->type) }}</span>
                                <span class="text-xs text-slate-500">{{ $request->start_time->format('d M, Y') }}</span>
                            </div>
                        </td>
