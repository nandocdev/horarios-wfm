    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <flux:card class="p-0 overflow-hidden">
            ...
            <div class="p-4 grid grid-cols-2 gap-4">
                ...
                <div
                    class="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-md border border-amber-100 dark:border-amber-900/30">
                ...
                <div
                    class="p-4 bg-green-50 dark:bg-green-900/10 rounded-md border border-green-100 dark:border-green-900/30">
                ...
                <div class="flex items-center gap-4">
                    <div class="flex-1">
                        <div class="flex justify-between mb-1">
                            <flux:text size="sm" class="font-bold uppercase tracking-tight">{{ $type['type'] ?: 'Otro' }}
                            </flux:text>
                            <flux:text size="xs" class="text-slate-400">{{ $type['count'] }}</flux:text>
                        </div>
                        <div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2">
                            <div class="bg-slate-600 h-2 rounded-full"
                                style="width: {{ ($type['count'] / max($leaveStats['total'], 1)) * 100 }}%"></div>
                        </div>
                    </div>
                </div>
            @endforeach
