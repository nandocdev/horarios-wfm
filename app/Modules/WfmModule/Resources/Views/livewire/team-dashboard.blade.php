<div class="space-y-4">
    {{-- Team Selector --}}
    <div
        class="flex flex-wrap items-center justify-between gap-3 p-4 bg-white dark:bg-zinc-900 border border-wfm-surface-border rounded-xl">
        <div class="flex items-center gap-3">
            ...
            <flux:select wire:model.live="selectedDate" class="w-44">
                @for($i = 0; $i <= 6; $i++)
                    ...
                    <flux:select.option value="{{ $d->toDateString() }}">
                        {{ $i === 0 ? 'Hoy' : $d->locale('es')->translatedFormat('l d F') }}
                    </flux:select.option>
                @endfor
            </flux:select>
        </div>
    ...
    @else
        ...
        <div class="p-4 bg-white dark:bg-zinc-900 border border-wfm-surface-border rounded-xl">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-semibold text-wfm-navy-800 dark:text-white uppercase tracking-wider">Headcount del
                    Equipo</span>
            ...
            <div class="flex h-6 rounded-full overflow-hidden mb-2">
                <div class="bg-wfm-success transition-all" style="width: {{ $pPct }}%"
                    title="Presentes: {{ $h['present'] ?? 0 }}"></div>
                <div class="bg-blue-400 transition-all" style="width: {{ $vPct }}%"
                    title="Vacaciones: {{ $h['vacation'] ?? 0 }}"></div>
                <div class="bg-amber-400 transition-all" style="width: {{ $lPct }}%"
                    title="Permisos: {{ $h['leave'] ?? 0 }}"></div>
                <div class="bg-wfm-danger transition-all" style="width: {{ $aPct }}%"
                    title="Ausentes: {{ $h['absent'] ?? 0 }}"></div>
                <div class="bg-purple-400 transition-all" style="width: {{ $sPct }}%"
                    title="Cambio Turno: {{ $h['swap'] ?? 0 }}"></div>
            </div>
            <div class="flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-wfm-surface-muted">
                <span class="flex items-center gap-1"><span class="w-2 h-2 rounded inline-block bg-wfm-success"></span>
                    Presentes {{ $h['present'] ?? 0 }}</span>
                <span class="flex items-center gap-1"><span class="w-2 h-2 rounded inline-block bg-blue-400"></span>
                    Vacaciones {{ $h['vacation'] ?? 0 }}</span>
                <span class="flex items-center gap-1"><span class="w-2 h-2 rounded inline-block bg-amber-400"></span>
                    Permisos {{ $h['leave'] ?? 0 }}</span>
                <span class="flex items-center gap-1"><span class="w-2 h-2 rounded inline-block bg-wfm-danger"></span>
                    Ausentes {{ $h['absent'] ?? 0 }}</span>
                <span class="flex items-center gap-1"><span class="w-2 h-2 rounded inline-block bg-purple-400"></span>
                    Cambio Turno {{ $h['swap'] ?? 0 }}</span>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <x-wfm.kpi :value="($k['connected_count'] ?? 0) . '/' . ($k['total_count'] ?? 0)" label="Conectados"
                ...
                <div class="flex items-end gap-1 h-24">
                    @foreach(['TALKING', 'READY', 'NOT_READY', 'WORK', 'RESERVED', 'NOT_READY_LUNCH', 'NOT_READY_BREAK', 'OFFLINE'] as $key)
                        ...
                        <div class="flex-1 flex flex-col items-center gap-1">
                            ...
                            <div class="w-full rounded-sm {{ $colorMap[$key] }}" style="height: {{ $hPx }}px; min-height: 4px;">
                            </div>
            ...
            <x-wfm.section title="Alertas Activas">
                <div class="space-y-1.5 max-h-48 overflow-y-auto">
                    @forelse($alerts as $alert)
                        <div
                            class="flex items-start gap-2 p-2 rounded text-xs {{ $alert['level'] === 'danger' ? 'bg-wfm-danger/5 border border-wfm-danger/20' : 'bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/20' }}">
                            <span class="mt-0.5">
        ...
        <x-wfm.section title="AHT por Agente y Cola">
            <div wire:key="aht-chart-{{ $teamId }}-{{ $selectedDate }}">
                <x-apex-chart id="aht-queue-chart" :options="$ahtChartOptions" height="290px" />
            </div>
        </x-wfm.section>
                    ...
                    <flux:table.row>
                        ...
                        <flux:table.cell>
                            <span class="inline-flex items-center gap-1">
                                <span
                                    class="w-2 h-2 rounded-full {{ $row['is_connected'] ? 'bg-wfm-success' : 'bg-wfm-danger' }}"></span>
                        ...
                        <flux:table.cell>
                            <div class="flex items-center gap-1.5">
                                ...
                                <span
                                    class="font-mono text-[10px] {{ $row['productivity'] >= 80 ? 'text-wfm-success' : ($row['productivity'] >= 60 ? 'text-wfm-warning' : 'text-wfm-danger') }}">{{ $row['productivity'] }}%</span>
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
                @empty
