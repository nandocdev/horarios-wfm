...
final class CachePolicyService
{
    ...
    public function flushByPattern(string $module, ?string $category = null): void
    {
        ...
        // NOTA: Esto requiere acceso directo a Redis. Para otros drivers es no-op.
        try {
            ...
            // Fallback: si no hay Redis (extensión faltante, driver database, servidor caído), no propagar
            Log::warning(
    ...
    public function getStats(): array
    {
        ...
        try {
            ...
            if (! method_exists($store, 'getRedis')) {
                $stats['error'] = 'Redis store no disponible (driver: '.config('cache.default').')';
