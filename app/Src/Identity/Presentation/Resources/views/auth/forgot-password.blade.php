<x-layouts::auth :title="__('¿Olvidaste tu contraseña?')">
    <div class="flex flex-col gap-6">
        ...
        <form method="POST" action="{{ route('identity.password.email') }}" class="flex flex-col gap-6">
            @csrf
        ...
        <div class="space-x-1 rtl:space-x-reverse text-center text-sm text-zinc-600 dark:text-zinc-400">
            ...
            <flux:link :href="route('identity.login')" wire:navigate>{{ __('Inicia sesión') }}</flux:link>
        </div>
    </div>
</x-layouts::auth>
