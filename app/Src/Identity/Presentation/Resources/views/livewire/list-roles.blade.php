<div class="space-y-6">
                    ...
                    <flux:table.row :key="$role->id">
                        <flux:table.cell>
                            <flux:badge size="sm" :color="match(true) {
                                ...
                                default => 'zinc',
                            }" variant="subtle" inset="top bottom">
                                Lvl {{ $role->hierarchy_level }}
                            </flux:badge>
                        </flux:table.cell>
                        <flux:table.cell variant="strong">{{ $role->name }}</flux:table.cell>
                        <flux:table.cell class="font-mono text-xs uppercase text-zinc-500">{{ $role->code }}</flux:table.cell>
                        <flux:table.cell>
                            <flux:text size="sm" class="text-zinc-500">{{ $role->permissions_count }} asignados</flux:text>
                        </flux:table.cell>
                        <flux:table.cell>
                            <flux:text size="sm" class="text-zinc-500">{{ $role->users_count }} usuarios</flux:text>
                        </flux:table.cell>
                        <flux:table.cell align="end">
                            <flux:button.group>
                                @can('roles.edit')
                                    <flux:button wire:click="editPermissions({{ $role->id }})" variant="filled" size="xs" icon="shield-check">
                                        Gestionar
                                    </flux:button>
                                @endcan
                                @can('roles.delete')
                                    <flux:button wire:click="delete({{ $role->id }})" variant="ghost" size="xs"
                                        class="text-red-500 hover:text-red-600" icon="trash"
                ...
                @empty
                    <flux:table.row>
                        <flux:table.cell colspan="6" class="text-center py-12">
                            <flux:text size="sm" class="italic text-zinc-400">No se han registrado roles institucionales todavía.</flux:text>
                        </flux:table.cell>
                    </flux:table.row>
                @endforelse
            </flux:table.rows>
    ...
    <flux:modal name="role-permissions" class="md:max-w-4xl">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Gestionar Permisos</flux:heading>
            ...
            <form wire:submit="savePermissions" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-h-[60vh] overflow-y-auto px-1">
                    @foreach($availablePermissions as $module => $perms)
                        <div class="bg-zinc-50 dark:bg-white/5 p-4 rounded-xl border border-zinc-100 dark:border-white/10 space-y-3">
                            <flux:text class="font-bold text-zinc-900 dark:text-zinc-100 uppercase text-[10px] tracking-widest opacity-70">
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-zinc-100 dark:border-white/10">
                    <flux:button x-on:click="$flux.modal('role-permissions').close()" variant="ghost">Cancelar</flux:button>
                    <flux:button type="submit" variant="primary" wire:loading.attr="disabled">
                        <span wire:loading.remove wire:target="savePermissions">Guardar Permisos</span>
                        <flux:icon name="arrow-path" class="w-4 h-4 animate-spin" wire:loading wire:target="savePermissions" />
