<div class="space-y-6">
    ...
    <div x-data="{ tab: 'news' }" class="space-y-6">
        <div class="flex space-x-4 border-b border-zinc-200 dark:border-zinc-700 pb-px">
            <button type="button" @click="tab = 'news'" :class="tab === 'news' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300'" class="px-4 py-2 text-sm font-medium border-b-2 transition-colors flex items-center gap-2">
                <flux:icon name="newspaper" class="w-4 h-4" />
                Noticias ({{ $pendingNews->total() }})
            </button>
            <button type="button" @click="tab = 'polls'" :class="tab === 'polls' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300'" class="px-4 py-2 text-sm font-medium border-b-2 transition-colors flex items-center gap-2">
                <flux:icon name="presentation-chart-line" class="w-4 h-4" />
                Encuestas ({{ $pendingPolls->total() }})
            </button>
            <button type="button" @click="tab = 'shoutouts'" :class="tab === 'shoutouts' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300'" class="px-4 py-2 text-sm font-medium border-b-2 transition-colors flex items-center gap-2">
        ...
        <div x-show="tab === 'news'" class="space-y-4">
                ...
                <flux:card>
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-2">
                            <form method="POST" action="{{ route('platform.communications.moderation.approve') }}">
                                @csrf
                            ...
                            <flux:modal name="reject-news-{{ $news->id }}" class="md:w-[450px]">
                                <form method="POST" action="{{ route('platform.communications.moderation.reject') }}" class="space-y-6">
                                    @csrf
                                    <input type="hidden" name="content_type" value="news">
                                    <input type="hidden" name="content_id" value="{{ $news->id }}">
                                    <div>
                            ...
                            <form method="POST" action="{{ route('platform.communications.moderation.archive') }}">
                                @csrf
                                <input type="hidden" name="content_type" value="news">
                                <input type="hidden" name="content_id" value="{{ $news->id }}">
                                <flux:button type="submit" variant="ghost" size="sm" icon="archive-box">Archivar</flux:button>
                            </form>
                        </div>
                    </div>
                </flux:card>
            ...
            @endforelse
            <div class="mt-4">{{ $pendingNews->links() }}</div>
        </div>
        ...
        <div x-show="tab === 'polls'" x-cloak class="space-y-4">
                ...
                <flux:card>
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                <flux:badge size="sm" color="zinc">Administración</flux:badge>
            ...
            @endforelse
            <div class="mt-4">{{ $pendingPolls->links() }}</div>
        </div>
        ...
        <div x-show="tab === 'shoutouts'" x-cloak class="space-y-4">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-2 mt-3">
                                <flux:badge size="sm" color="indigo">Para: {{ $shoutout->employee->name ?? $shoutout->employee->full_name }}</flux:badge>
            ...
            @endforelse
            <div class="mt-4">{{ $pendingShoutouts->links() }}</div>
        </div>
    </div>
</div>
