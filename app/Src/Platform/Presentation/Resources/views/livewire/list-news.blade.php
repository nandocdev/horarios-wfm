<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <flux:button href="{{ route('platform.communications.news.create') }}" variant="primary" icon="plus" wire:navigate>
            Nueva Noticia
        </flux:button>
    </div>
                    ...
                    <flux:table.row :key="$item->id">
                        ...
                        <flux:table.cell align="end">
                            <div class="flex justify-end gap-2">
                                <flux:button href="{{ route('platform.communications.news.edit', $item) }}" variant="ghost" icon="pencil-square" size="sm" wire:navigate />
                                <flux:button wire:click="deleteNews({{ $item->id }})" wire:confirm="¿Estás seguro de eliminar esta noticia?" variant="ghost" color="red" icon="trash" size="sm" />
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
                @empty
