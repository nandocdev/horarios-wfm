<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <flux:button href="{{ route('platform.communications.polls.create') }}" variant="primary" icon="plus" wire:navigate>
            Nueva Encuesta
        </flux:button>
    </div>
                            ...
                            <div class="flex justify-end gap-2">
                                ...
                                <flux:modal name="results-poll-{{ $item->id }}" class="md:w-[500px]">
                                    <div class="space-y-6">
                                        ...
                                        <div class="space-y-4">
                                            @php $totalVotes = collect($item->options)->sum(fn($opt) => $opt['votes'] ?? 0); @endphp
                                ...
                                @if($item->status === 'published')
                                    <flux:button wire:click="archivePoll({{ $item->id }})" wire:confirm="¿Cerrar esta encuesta?" variant="ghost" icon="archive-box" size="sm" tooltip="Cerrar Encuesta" />
                                @endif

                                <flux:button wire:click="deletePoll({{ $item->id }})" wire:confirm="¿Eliminar encuesta?" variant="ghost" color="red" icon="trash" size="sm" />
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
                @empty
