<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <flux:button href="{{ route('platform.communications.shoutouts.create') }}" variant="primary" icon="plus" wire:navigate>
    ...
    <flux:card>
                    ...
                    <flux:table.row :key="$item->id">
                        ...
                        <flux:table.cell align="end">
                            <div class="flex justify-end gap-2">
                                <flux:button href="{{ route('platform.communications.shoutouts.edit', $item) }}" variant="ghost" icon="pencil-square" size="sm" wire:navigate />
                                <flux:button wire:click="deleteShoutout({{ $item->id }})" wire:confirm="¿Eliminar este reconocimiento?" variant="ghost" color="red" icon="trash" size="sm" />
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
                @empty
        ...
        <div class="mt-4">{{ $shoutouts->links() }}</div>
    </flux:card>
</div>
