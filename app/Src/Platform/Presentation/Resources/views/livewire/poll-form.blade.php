<div class="space-y-6">
    ...
    <form wire:submit="save" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div class="lg:col-span-8 space-y-6">
            <flux:card>
                <div class="space-y-4">
                    <flux:input wire:model="form.question" label="Pregunta de la Encuesta" placeholder="Ej. ¿Qué te parece el nuevo horario de almuerzo?" />
                    ...
                    <div class="space-y-3">
                        @foreach($form['options'] as $index => $option)
                            <div class="flex items-end gap-3" wire:key="option-{{ $index }}">
                                <div class="flex-1">
                                    <flux:input wire:model="form.options.{{ $index }}.text" label="Opción {{ $index + 1 }}" placeholder="Escribe una respuesta..." />
                                </div>
                                <div class="w-32">
                                ...
                                @if(count($form['options']) > 2)
                                    <flux:button variant="ghost" icon="trash" size="sm" color="red" wire:click="removeOption({{ $index }})" />
                                @endif
                            </div>
                        @endforeach

                        @if(count($form['options']) < 10)
                            <flux:button variant="outline" icon="plus" size="sm" class="w-full mt-2" wire:click="addOption">Agregar otra opción</flux:button>
                        @endif
                    </div>
                </div>
            </flux:card>
        ...
        <div class="lg:col-span-4 space-y-6">
            ...
            <div class="flex gap-3">
                <flux:button variant="ghost" class="flex-1" href="{{ route('platform.communications.polls.index') }}" wire:navigate>Cancelar</flux:button>
                <flux:button variant="primary" type="submit" class="flex-1">Guardar Encuesta</flux:button>
            </div>
        </div>
    </form>
</div>
